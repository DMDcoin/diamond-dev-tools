...
async function runSmallTestNetwork() {
  ...
  console.log(
    `Early epoch test network. designed to run on network ${testname}. create with 'npm run testnet-fresh-${testname}' .`
  );
  ...
  console.log(
    `waiting for next epoch switch and upscaling to 4 validator nodes.`
  );

  const maxExpectedTestDuration = duration(3, "minutes");

  const maxExpectedTestEndDate =
    Date.now() + maxExpectedTestDuration.asMilliseconds();
  let isSuccess = true;
  ...
  while (currentValidators.length < 4) {
    ...
    if (Date.now() > maxExpectedTestEndDate) {
      console.log(
        "ABORTING: test took too long. expected to finish in ",
        maxExpectedTestDuration.asMinutes(),
        " minutes"
      );
      isSuccess = false;
      break;
