...
export async function createBlock(
  web3: Web3,
  last_checked_block: number = Number.NaN
) {
  ...
  if (balance.lt(web3.utils.toBN(1000000000000000000))) {
    console.warn(
      "Not enough balance to send transaction to trigger block creation: missconfiguration ??"
    );
  }

  console.log(
    "sending transaction to trigger block creation, main account balance:",
    balance.toString()
  );
