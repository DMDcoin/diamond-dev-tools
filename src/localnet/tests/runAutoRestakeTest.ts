...
class RestakeObservation {
  public constructor(
    ...
    public rewardBlockNumber: number,
    public countOfDelegators: number,
...
class AutoRestakeTest {
  ...
  public async runTest(numOfDelegatorsEachEpoch: number = 1000) {
    ...
    console.log(
      "Tests and documents the implication of the automatic reward restaking feature https://github.com/DMDcoin/diamond-contracts-core/issues/43"
    );
    ...
    let deltaFeedTX = await rewardContract.methods
      .addToDeltaPot()
      .send({
    ...
    watchdog.onEpochSwitch = async (epoch: number, blockNumber: number) => {
      ...
      validators = await contractManager.getValidators();
      console.log(
        "validators after epoch switch: ",
        validators.length,
        "/",
        targetNumOfValidators
      );

      if (validators.length != targetNumOfValidators) {
        if (isInitialised) {
          console.log(
            "detected unexpected shrink of the validator set - aborting!",
            validators.length
          );
          shutdown();
          process.exit(1);
      ...
      if (!isInitialised) {
        ...
        for (let validator of validators) {
          poolAddresses.push(
            await contractManager.getAddressStakingByMining(validator)
          );
        }
      ...
      let timeConsumed =
        toNumber(blockForEpochSwitch.timestamp) -
        toNumber(blockBeforeEpochSwitch.timestamp);
      console.log("last epoch switch took: ", timeConsumed, " seconds");

      let totalStake = BigInt(
        await web3.eth.getBalance(stakingContract.options.address)
      );
      let obsersvation = new RestakeObservation(
        epoch,
        blockNumber,
        totalDelegatorsCount,
        timeConsumed,
        lastTotalGasConsumption,
        totalStake
      );
      ...
      for (let i = 0; i < numOfDelegatorsEachEpoch; i++) {
        ...
        let minStakeForDelegators = web3.utils.toBN(
          await stakingContract.methods.delegatorMinStake().call()
        );
        let minStakeForDelegatorsAndFees = minStakeForDelegators.add(
          web3.utils.toBN(web3.utils.toWei("1", "ether"))
        );

        console.log(
          "funding account with nonce: ",
          nonce,
          "address:",
          account.address
        );
        web3.eth
          .sendTransaction({
            from: web3.eth.defaultAccount!,
            to: account.address,
            value: minStakeForDelegatorsAndFees.toString(),
            gas: "21000",
            nonce: nonce,
          })
          .once("receipt", (receipt) => {
            console.log(
              "receipt: did receive funds for ",
              account.address,
              " tx: ",
          ...
          .once("confirmation", async (payload) => {
            ...
            const transactionConfig = stakingContract.methods
              .stake(targetAddress)
              .encodeABI();
    ...
    console.log(
      "Test Finished, reached target delegators count of ",
      targetMaxDelegators
    );
    shutdown();
  }
