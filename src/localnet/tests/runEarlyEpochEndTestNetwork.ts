...
async function runEarlyEpochTestNetworkOld() {
  console.log(
    `Early epoch test network. designed to run on network nodes-local-test-early-epoch-end. create with 'npm run testnet-fresh-test-early-epoch-end'.`
  );
  ...
  bonusScoreSystem.events.ValidatorScoreChanged((error, result) => {
    ...
    if (result) {
      ...
      console.log(
        `ValidatorScoreChanged # ${result.blockNumber} pool ${result.address}, mining: ${v.miningAddress} factor: ${v.factor} new score: ${v.newScore} `
      );
    }
  });
  ...
  console.log(
    `Epoch number at start: ${current_epoch} block:  ${last_checked_block}`
  );
  await createBlockAndRefresh();
  console.log("block creation confirmed.");
  console.log(
    `waiting for next epoch switch and upscaling to 16 validator nodes.`
  );
  ...
  console.log(
    "node 1 stopped, creating block should work, because of fault tolerance."
  );
  ...
  console.log(
    `FAILURE: Epoch switch did not happen within the expected time of seconds: `,
    maxTriesForEpochSwitch
  );
